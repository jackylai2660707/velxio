...
| 2.AI | **TIMG DIVIDER respect** — counter time base now scales with bits 21:9 of T0_CONFIG (was hardcoded 1 MHz). `tick_rate = APB(80MHz) / DIVIDER`. Self-test now uses DIVIDER=8000 → 10 kHz tick, counter values 100× smaller while wall-clock cadence preserved. | ✅ done | (this commit) | [phase_2ai_timg_divider.md](phase_2ai_timg_divider.md) |
| **next** | TBD: 2.AJ (guest ISR demo — install mtvec stub, ISR toggles GPIO pin on TIMG IRQ), TIMG1 + WDT, I2C master, SPI master, real PWM waveform on GPIO, or real FreeRTOS port (Phase 2.V deferred) | ⏭️ choice | — | — |
