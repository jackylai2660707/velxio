...
| 2.AW | **UART TX JSON event tracking** — overrides ESP32P4 UART class `uart_write` virtual to emit `event:"uart_tx"` for every TX FIFO write. 26 events at boot decode as ASCII spelling "Hello from QEMU ESP32-P4!". Frontend can now render `Serial.print()` output as a serial-monitor view. Pattern: chain to parent's saved write pointer + emit JSON. No throttling — every byte matters. | ✅ done | (this commit) | [phase_2aw_uart_tx.md](phase_2aw_uart_tx.md) |
| **next** | TBD: 2.AW.rx (UART RX JSON tracking), WDT actual reset, real PWM, TWAI (CAN), RMT (WS2812), FreeRTOS (deferred) | ⏭️ choice | — | — |
