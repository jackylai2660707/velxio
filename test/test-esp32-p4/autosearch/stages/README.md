...
| 2.V | **3-pin running light @ ~3.5 Hz** cycling pins 5→6→7 via trampoline-to-blob in cache-window IRAM. Discovered: L2MEM is NOT executable under IDF PMP rules; cache window 0x40000000+ is the safe executable region. | ✅ done | (this commit) | [phase_2v_running_light.md](phase_2v_running_light.md) |
| **2.W** | **GPIO input register handling** (R_GPIO_IN at 0x3C) + external "button" device for ISR demos | ⏭️ **next** | — | TBD |
