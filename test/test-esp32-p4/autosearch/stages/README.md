...
| 2.X | **GPIO event stream** to JSON-Lines file (env-gated `VELXIO_GPIO_LOG`). Each pin transition appends `{"t_ns":...,"pin":N,"level":M}`. Wall-clock timestamps. Frontend tail-f-able. Default build unchanged. | ✅ done | (this commit) | [phase_2x_event_stream.md](phase_2x_event_stream.md) |
| **2.X.input** | **Reverse channel**: frontend writes JSON events → emulator forwards to GPIO input pads. Lets user click virtual buttons in web UI. | ⏭️ **next** | — | TBD |
