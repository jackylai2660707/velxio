...
| 2.AH | **TIMG → CPU IRQ wiring (cause 19)**. INT_RAW & INT_ENA propagates via `qdev_connect_gpio_out_named` to `espressif-cpu-irq-lines`. Live test confirms IRQ line transitions 0→1 at first alarm match (`event:"timg_irq"`). Self-test enables INT_ENA; CPU has MIE=0 in bypass flow so no trap regression. | ✅ done | (this commit) | [phase_2ah_timg_irq.md](phase_2ah_timg_irq.md) |
| **next** | TBD: 2.AI (guest ISR demo — install mtvec stub, ISR toggles GPIO pin on TIMG IRQ), TIMG1 + WDT, I2C master, SPI master, real PWM waveform on GPIO, or real FreeRTOS port (Phase 2.V deferred) | ⏭️ choice | — | — |
