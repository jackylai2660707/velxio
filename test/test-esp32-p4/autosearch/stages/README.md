...
| 2.T-fix.next.next | **Print::write virtual dispatch neutralised** at `0x4000079A` (NULL vtable). New blocker: `uxListRemove` called with garbage pointer (FreeRTOS state corruption from skipped scheduler init). Each subsequent fault is deeper in FreeRTOS — single-patch chain not viable. | ✅ done w/ findings | (this commit) | [phase_2t_fix_next_next_print_write.md](phase_2t_fix_next_next_print_write.md) |
| **2.U (re-scoped)** | **Isolate LED-blink subset**: replace loopTask body with hand-rolled asm that does only `digitalWrite + busy_delay`, no Print/Serial/FreeRTOS. QEMU GPIO log shows toggles. | ⏭️ **next** | — | TBD |
| 2.V | Real FreeRTOS port emulation (long multi-week effort, parallel track) | ⏳ planned | — | TBD |
