...
| 2.AJ | **ISR + mtvec + MIE install** in demo blob (5 init + 8 ISR instructions). Self-test → one-shot for safety. Pin 8 added to ENABLE mask. **CPU does NOT trap to ISR** on TIMG IRQ — Phase 2.S CLIC mode dispatch likely intercepts. Infrastructure verified install-correct, but needs Phase 2.AK to diagnose CLIC dispatch path. | 🔬 done w/ findings | (this commit) | [phase_2aj_isr_demo_attempt.md](phase_2aj_isr_demo_attempt.md) |
| **next** | TBD: 2.AK (diagnose CLIC dispatch — try mtvt[19] write or clicintctl[19] enable so ISR actually fires), TIMG1 + WDT, I2C master, SPI master, real PWM waveform on GPIO, or real FreeRTOS port (Phase 2.V deferred) | ⏭️ choice | — | — |
