...
- 教師在 `/teacher` 建立作業，可設定類型(專題、測驗、反思)、說明、關聯課次、
  截止時間、滿分與是否自動評分。
- 作業先儲存為草稿，再按「發布」；教師可在收件視窗查看提交狀態、自動分數、
  教師分數與回饋。
- 學生在 `/learn` 看到所屬班級的已發布作業，可提交選擇題、文字回覆與目前
  `.vlx` 工作區；測驗提交後即時自動評分，專題/反思可由教師補評。
- 作業資料與提交記錄存於 SQLite，支援截止時間、狀態、草稿與重複提交；大檔案
  仍應使用學生的雲端專案儲存，不把無限制檔案塞進作業表。

## 後續方向

- 成績報表 CSV / XLSX 匯出與批量評語。
- 更完整的 rubric 評分、附件預覽與作業重開放。
