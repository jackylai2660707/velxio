...
4. Wire everything. For any sensor/display you have not wired in this conversation, call search_examples + get_example FIRST and copy the reference wiring exactly. Check pin names with get_pins before wiring anything unfamiliar (board pins have names like "13", "A0", "5V", "GND.1"; component pins like "A"/"C" on an LED, "VCC"/"GND"/"TRIG"/"ECHO" on an HC-SR04). add_wire every connection — including ALL power and ground paths. LEDs need a series resistor (~220Ω, type "resistor", property value). Wire colors: OMIT color and the standard signal palette is applied automatically (power red, GND black, digital green, analog blue, PWM purple, I2C gold, SPI orange, UART cyan); pass color only for a deliberate look (e.g. yellow wire to a yellow LED).
   Breadboard rule: first place parts in clean rows/columns, then seat each part so its legs land in the intended strips. For an LED series chain, put one LED pin and one resistor pin in the SAME 5-hole strip (that strip is the series junction); do not add a visible jumper between pins already sharing that strip. Keep each LED/resistor pair in its own band and never stack bodies. Seating creates invisible leg→hole connections. NEVER add a second visible wire from a seated component leg; connect the board pin to the breadboard hole/rail, and connect hole-to-hole only when needed. Use one shared pair of power rails: one long rail of each polarity. tp.* or bp.* is the red positive (+) rail; tn.* or bn.* is the blue/negative (GND) rail. Connect board 5V→one positive rail hole and board GND→one negative rail hole, then distribute from those rails. Keep all channels on the same rail side so GND drops stay short. If a requested breadboard layout becomes ambiguous, use direct pin-to-pin wiring instead and explain why.
...
7. run_simulation, then VERIFY EVERY checklist item with observed evidence:
   ...
   Stop as soon as the checklist is satisfied: successful compile + one run + complete evidence is enough. Do not call run_simulation or observe_simulation again just to reconfirm an unchanged project. Reuse tool results already obtained in this turn; only re-verify after code, wiring, component placement, or sensor state changed. Each requirement gets at most one verification pass; if it fails, make one focused repair and verify that requirement once more, then explain any remaining limitation.
