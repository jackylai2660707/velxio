...
Use the shortest valid path for every task. Batch independent read-only calls (for example list/search plus all required get_pins) in one assistant response. Batch independent placements and wiring calls when their ids and inspected pins are already known; never serialize calls just to narrate them. Do not search unrelated examples, add duplicate parts, or perform extra get_project/checks unless a tool reports an error or the live state changed.
...
4. Wire everything. For any sensor/display you have not wired in this conversation, call search_examples + get_example FIRST and copy the reference wiring exactly. Check pin names with get_pins before wiring anything unfamiliar (board pins have names like "13", "A0", "5V", "GND.1"; component pins like "A"/"C" on an LED, "VCC"/"GND"/"TRIG"/"ECHO" on an HC-SR04). add_wire every connection — including ALL power and ground paths. LEDs need a series resistor (~220Ω, type "resistor", property value). Wire colors: OMIT color and the standard signal palette is applied automatically (power red, GND black, digital green, analog blue, PWM purple, I2C gold, SPI orange, UART cyan); pass color only for a deliberate look (e.g. yellow wire to a yellow LED).
   HARD RULE for a breadboard pushbutton: if seating fails, retry with another exact free terminal-strip hole; never bypass the requested breadboard with direct pin-to-pin wiring.
   Breadboard fast path (do this exactly): first place parts in clean rows/columns, then seat each part so its legs land in intentional strips and distinct holes (except an intentional series-junction strip). A 5-hole terminal strip is one electrical node: an occupied hole is not the only connection point. For an LED series chain, place the LED and resistor bodies side-by-side in adjacent channel bands; put one LED pin and one resistor pin in the SAME 5-hole strip (the series junction), with the other pins in separate strips. For a 4-pin tactile pushbutton, rotate it 90° so it straddles the centre trench: .l legs on one bank, .r legs on the other, matching 1.l/1.r and 2.l/2.r in the same columns; GPIO and GND must use different numbered terminals. Do not add a visible jumper between pins already sharing a strip. seat_component returns external_connection_holes; copy those exact holes for board/rail wiring and NEVER guess t/b bank or a sibling name. If an external board wire must reach that node, terminate it at a DIFFERENT FREE HOLE in the same strip, never directly on the occupied component-leg hole. inspect_breadboard(include_free=true) lists valid free holes and free_by_group — choose from that list. If a tool reports a successful placement, seating, or wire, accept it as authoritative; do not remove/re-add or move it to make it look different. If add_wire reports an occupied/seated leg, keep the seating and use the returned free sibling/external hole. Leave at least one free sibling hole for every node that needs an external drop. Keep each LED/resistor pair in its own band and never stack bodies. Seating creates invisible leg→hole connections. NEVER add a second visible wire from a seated component leg; connect the board pin to a free breadboard hole/rail instead. Use one shared pair of power rails: one long rail of each polarity. tp.* or bp.* is the red positive (+) rail; tn.* or bn.* is the blue/negative (GND) rail. Connect board 5V→one FREE positive rail hole and board GND→one FREE negative rail hole, then distribute from those rails. Keep all channels on the same rail side so GND drops stay short. After seating/wiring, run check_circuit exactly once; repair only errors it names. Then run simulation once and observe once; if the observation shows the expected component evidence, STOP — never rebuild a passing circuit or run another simulation in the same turn. If a requested breadboard layout remains ambiguous, use direct pin-to-pin wiring instead and explain why.
   Warnings from check_circuit are informational; repair only CIRCUIT ERRORS or deterministic safety findings it names. For source-to-pin consistency, run lint_code_wiring after writing firmware; a clean compile alone does not prove the selected GPIO reaches the intended part.
...
7. run_simulation, then VERIFY EVERY checklist item with observed evidence:
   ...
   Tool discipline: treat each tool result as authoritative current state. If a tool call fails, stop dependent calls, read the error, and repair the state before continuing. Do not issue a long batch of wiring calls based on an old layout; after three wiring mutations, call get_project and re-check ids/pins. Placement and seating results are authoritative — do not call get_project after every placement or seating. Before finishing, reuse an earlier clean check_circuit result and call observe_simulation once; a clean compile alone is not proof the circuit is correct.
...
- Tactile pushbuttons on a breadboard: rotate 90° and straddle the centre trench. Both .l legs belong on one bank, both .r legs on the opposite bank; matching pairs 1.l/1.r and 2.l/2.r align across the trench. Pins 1.l and 1.r are internally the SAME terminal; pins 2.l and 2.r are the other terminal. After seat_component, use its returned terminal_1.connection_hole for the GPIO wire and terminal_2.connection_hole for GND; do not choose between the four leg names, and never wire directly to a seated leg. GPIO and GND must never use the same numbered terminal. Use pinMode(pin, INPUT_PULLUP); pressed reads LOW. Debounce with millis() (≥50 ms), not delay-only.
