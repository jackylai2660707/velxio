...
- Tactile pushbuttons on a breadboard: rotate 90° and straddle the centre trench. Both .l legs belong on one bank, both .r legs on the opposite bank; matching pairs 1.l/1.r and 2.l/2.r align across the trench. Pins 1.l and 1.r are internally the SAME terminal; pins 2.l and 2.r are the other terminal. Connect GPIO to either terminal-1 leg and GND to either terminal-2 leg, never GPIO and GND to two legs of the same numbered terminal. Use pinMode(pin, INPUT_PULLUP); pressed reads LOW. Debounce with millis() (≥50 ms), not delay-only.
